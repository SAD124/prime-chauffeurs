...
const MiniNavbar = () => {
  ...
  return (
    <nav className="md:hidden fixed top-0 w-full z-50">
      {/* TOP BAR */}
      <div className=" flex items-center justify-between py-2 px-4 bg-white border-b border-gray-100">
        {/* MENU TOGGLE */}
      ...
      {/* SIDEBAR MENU */}
      <div
        className={`fixed left-0 top-20 h-[calc(100%-70px)] bg-white shadow-lg w-64 transform transition-transform duration-300 ease-in-out ${
          open ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        <div className="flex flex-col px-8 py-10 space-y-8">
          <HashLink smooth to="/#ridesmini" onClick={() => setOpen(false)} className="flex items-center gap-2">
            <Car className="w-5 h-5" /> Rides
          </HashLink>

          <HashLink smooth to="/#servicesmini" onClick={() => setOpen(false)} className="flex items-center gap-2">
            <Briefcase className="w-5 h-5" /> Services
          </HashLink>

          <HashLink smooth to="/#aboutmini" onClick={() => setOpen(false)} className="flex items-center gap-2">
            <Info className="w-5 h-5" /> About
          </HashLink>

          <HashLink smooth to="/#appmini" onClick={() => setOpen(false)} className="flex items-center gap-2">
