  ...
  return (
    <section className="w-full bg-[#1E1E1E] flex flex-col items-center py-12 sm:py-20 px-4 gap-8 border-t border-[#D4C5A0] md:hidden">
      ...
      <div className="flex flex-col items-center gap-4 text-center ">
        ...
        <p className="text-gray-400 font-roboto text-sm leading-[150%]">
      ...
      <div className="flex flex-col sm:flex-row gap-6 items-stretch sm:items-center sm:justify-center w-full">
        <Link
          ...
          className="rounded-md border border-[#D4C5A0] text-white px-4 py-2 w-full sm:w-auto font-roboto flex justify-center items-center
                     hover:bg-white hover:text-black transition-colors duration-300 text-sm"
        >
          Hire our rider
        </Link>

        <button className="flex items-center justify-center gap-2 font-roboto text-[14px] sm:text-[16px] group">
          <span className="text-white block transition-transform transform duration-300 group-hover:scale-110 group-hover:text-[#D4C5A0]  text-sm">
            Learn more
          </span>
          <IoChevronForward
            size={14}
            className="text-white transition-colors duration-300 group-hover:text-[#D4C5A0]"
          />
        </button>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 sm:gap-8 items-center">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            className="font-roboto text-[13px] text-white transition-all duration-300 transform hover:scale-110 hover:text-[#D4C5A0]"
          >
            {tab.label}
          </button>
        ))}
